...
export function SettingsPage({ continuousListening, speechAvailable, offlineSpeech, voices = [], voiceUri, onVoiceChange, onPreviewVoice, neuralVoice, onNeuralVoiceChange, onInstallNeuralVoice, onListeningChange, onOfflineSpeechChange, onResetDefaults, onEraseAll }: Props) {
  ...
  // While the neural voice reads the cards, nothing in the system-voice picker
  // affects playback, so it must not look like it does.
  const systemVoiceInactive = Boolean(neuralVoice?.enabled && neuralVoice.installed)
  return <div className="settings-grid">
    ...
    <div className={`voice-choice ${systemVoiceInactive ? 'is-inactive' : ''}`}>
      <label className="field"><span>Reading voice</span><select aria-label="Reading voice" disabled={systemVoiceInactive} value={voiceUri ?? ''} onChange={(event) => onVoiceChange?.(event.target.value)}>{voices.map((item) => <option key={item.uri} value={item.uri}>{item.name}</option>)}</select></label>
      {systemVoiceInactive && <p className="notice">Not in use: the high-quality voice above is doing the reading. Untick it to choose a system voice here.</p>}
      <button className="secondary-button" onClick={() => onPreviewVoice?.()}>🔊 Preview</button><p>Listed best first. Eddy, Flo, Grandma, Grandpa, Reed, Rocko, Sandy and Shelley are macOS character voices and sound artificial; Tingting is the natural Mandarin voice. For better quality, install an enhanced voice in System Settings → Accessibility → Spoken Content → System Voice → Manage Voices.</p>
    </div><p className="notice">Speech recognition cannot reliably judge tone pitch. Audio may be sent to your browser vendor for processing, but this app never stores recordings or transcripts.</p></section>
